...
- "AMENDED in r7 — was: 'No LLM-authored filter query params — selection only, from
   loader-returned values.' Still binding for the SEARCH agent (/s), whose whole job is
   picking one filter out of a list the loader already produced. NOT binding for the
   personal shopping agent, which authors a CollectionBrief: a structured criterion the
   model composes rather than selects. The rule's purpose — making filter hallucination
   STRUCTURALLY impossible, not merely mitigated — is preserved by a different mechanism:
   every value inside a brief is checked against getCatalogVocabulary() and dropped if the
   catalog does not contain it, before any SQL is built. What changed is composition, not
   trust. See docs/personal-shopping-agent-proposta.md §15."
