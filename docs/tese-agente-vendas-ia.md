...
- "No LLM-authored filter query params — selection only, from loader-returned values.
   (An r7 amendment carving out the personal shopping agent was drafted on 2026-08-07 and
   WITHDRAWN on 2026-08-08: that agent no longer authors filters at runtime — it writes
   product_props and product_affinity rows offline, reviewed by a human before they become
   SQL. The design stopped needing the permission, so the permission was taken back. Full
   reasoning in docs/personal-shopping-agent-proposta.md §15.)"
