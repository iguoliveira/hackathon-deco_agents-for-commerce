    ...
    <Section.Container {...viewItemListEvent}>
      <div className="flex flex-col gap-1">
        ...
        {mostrarProcedencia && (
          ...
          // perguntar, e é a única coisa na tela que prova que o histórico entrou
          // na decisão. Sem cidade nem mês aqui, ao contrário do look: a vitrine
          // não depende de clima — depende de quem a pessoa é.
      ...
      >
        {pecas.map((peca, index) => (
          ...
          >
            <ProductCard index={index} product={peca.product} itemListName={nome} />
            {/* Sem guarda de motivo vazio: `validar` descarta peça sem motivo, e
                não existe vitrine sem texto — um card mudo aqui seria bug a ser
                visto, não caso a ser tolerado.

                min-h fixo: sem ele, um motivo de uma linha ao lado de um de duas
                desalinha a régua inteira da grade. */}
